...
Below there are three plots shown. Each one represents one test case.
...
\begin{tikzpicture}
\begin{axis}[
    title={Time of adding five devices to an application},
    xlabel={Device [nth]},
    ylabel={Time [s]},
    xmin=1, xmax=5,
    ymin=0, ymax=140,
...
\end{tikzpicture}
    ...
    \caption{The plot represents the results of testing the efficiency to add 20 devices to an application with three execution modes: manual, automated windowed mode and automated headless mode.
    The corresponding data values for each coordinate value can be found from Table~\ref{tab:20_devices}}\label{fig:20_devices}
\end{figure}
...
Further investigation through the ChirpStack forum resulted in finding people with similar problem when using the same gateway \cite{chirpstack_forum:redis_error}.
